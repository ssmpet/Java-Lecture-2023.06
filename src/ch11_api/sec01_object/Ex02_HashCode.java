package ch11_api.sec01_object;

import java.util.HashMap;
import java.util.Map;

public class Ex02_HashCode {

	public static void main(String[] args) {
		Key key1 = new Key(3);
		Key key2 = new Key(3);
		System.out.println(key1.equals(key2));
		System.out.println(key1.hashCode() + ", " + key2.hashCode());
		
		// hashCode() 메소드를 재정의 하기 이전
		Map<Key, String> hashMap = new HashMap();
		hashMap.put(key1, "James");
		hashMap.put(key2, "Maria");
		System.out.println(hashMap.get(key1));
		System.out.println(hashMap.get(key2));	
		
		// hashCode()를 재정의한 후 
		Map<Key2, String> hashMap2 = new HashMap();
		Key2 key3 = new Key2(3);
		Key2 key4 = new Key2(3);
		hashMap2.put(key3, "James");			// HashMap은 key가 중복되면 값을 새로이 저장하지 않음
		hashMap2.put(key4, "Maria");			// 기존 key가 가리키는 value를 변경함
		System.out.println(hashMap2.get(key3));	// Maria
		System.out.println(hashMap2.get(key4));	// Maria
		System.out.println(key3.hashCode() + ", " + key4.hashCode());

	}

}
